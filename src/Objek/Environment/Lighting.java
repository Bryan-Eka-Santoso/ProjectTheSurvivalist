...
public class Lighting {
    ...
    public void setLightSource() {
        ...
        for (Buildings b : gp.buildings) {
            if (b instanceof Torch) {
                Torch torch = (Torch) b;
                int torchX = torch.worldX + torch.width / 2;
                int torchY = torch.worldY + torch.height / 2;
                RadialGradientPaint gPain = new RadialGradientPaint(torchX, torchY, ((Torch) torch).lightRadius, fraction, color);

                g2.setPaint(gPain);
                g2.fillRect(torchX - ((Torch) torch).lightRadius, torchY - ((Torch) torch).lightRadius, ((Torch) torch).lightRadius * 2, ((Torch) torch).lightRadius * 2);
            }
        }
